...
\item[\kw{kernel} ($\nseq{d}{n}$)
  ...
  form of $n$ \kw{map}-nets, where nest number $i$ (counted from
  ...
  then the \kw{kernel} construct as a whole returns $m$ arrays, each
...
unique new \kw{manifest} array.\fixme{Not necessarily ideal/optimal.}
...
\StreamSeq{} are invariant to the same dimension.\fixme{show or remark
  on the streamSeq rewrite rule that allows one to re-write a
  streamSeq construct into a nest of two streamSeq constructs.}
...
(or $2$), resulting in a one-dimensional array.\fixme{very unclear; expand.  Maybe make this explicit.}
...
\fixme{fix here}you should say safety requires that the block size for
dimension "i"
