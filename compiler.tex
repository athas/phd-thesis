...
pipeline, as it is by far the more complicated of the two.

The pipeline proceeds with the following passes in order as follows:

\begin{description}
\item[Internalisation and Desugaring] The source Futhark
language is translated into the core language
(\cref{sec:futhark-core-ast}).  In particular, all tuples are
flattened, and arrays of tuples are transformed into tuples of arrays,
as shown on \ref{fig:calc-tuple-transform}.  This is also the pass
where where size inference is performed (\cref{chap:size-analysis}).
...
defines a rule that removes redundant \kw{reshape} operations.  This
is the sole exception to our general policy of not showing compiler
