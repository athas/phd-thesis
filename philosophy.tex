...
programming language running on a specific computer.  But in practise,
...
hope is that if the model can be mapped efficiently to a platform as
...
philosophy behind Futhark.  Specifically, although we do not mind
aggressive optimisation, the transformations we perform should arise
naturally from the constructs used by the programmer, and not depend
on subtle and fragile analyses.
...
simplification may be possible.  While wide-spread techniques such as
...
refer to the compiler having to reverse engineer users optimizations,
