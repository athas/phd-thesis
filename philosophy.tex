...
function on \Cref{fig:futhark-dotprod} requires that the two
input arrays have the same length, while the matrix multiplication on
\Cref{fig:futhark-matmult} specifies the usual size constraints
...
\begin{itemize}
    \item the implicit invariant \texttt{civ@4$^{i-1}\=$civ@2$^{i}$},
            i.e., the \texttt{civ} value at an iteration end is equal to 
            the \texttt{civ} value at the entry of the next iteration, and 
    \item the monotonicity of the \texttt{civ@2} values 
          (because they are incremented by \texttt{C(i)} only when
            \texttt{C(i)} is positive).
\end{itemize}
...
We conclude by remarking that in addition to ``heroic'' analysis 
necessary for proving the absence of cross-iteration dependences 
for array \texttt{X} that uses CIV-based accesses, 
the compiler also had to re-engineer the computation of \texttt{civ} 
values from inherently sequential to parallel. One could argue 
that, if parallel execution is desired, the code should have been 
written from the very beginning in terms of data-parallel operators
