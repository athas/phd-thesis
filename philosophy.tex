...
this means that the cost of the update is proportional to the size of
the array, not the part that we are updating.  With an in-place
update, we pay only for the part of the array that we are updating.
This section describes how Futhark supports in-place updates without
violating the referential purity that we depend on for safe parallel
programming.
...
\begin{lstlisting}
let modify [n] (a: *[n]int) (i: int) (x: [n]int): *[n]int =
  a with [i] <- (a[i] + x[i])
\end{lstlisting}
...
This code forms the sequential implementation.  While a small amount
