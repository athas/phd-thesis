...
\node (input) {\footnotesize\textit{xs}};
...
\draw[post] (map1) to  node[midway,xshift=2mm] {\footnotesize\textit{ys}} (map2);

\draw[post,draw,bend left,markedge] (reduce) to  node[pos=0.28,right] {\footnotesize\textit{sum}} (map2);

\node (ys) [below of=map2] {\footnotesize\textit{zs}}
   edge [pre] (map2);


  \begin{pgfonlayer}{background}
    \filldraw [line width=4mm,join=round,black!10]
      (map1.north -| reduce.east) rectangle (reduce.south -| map1.west)
      (map2.north -| map2.east) rectangle (map2.south -| map2.west);
  \end{pgfonlayer}
\end{tikzpicture}
