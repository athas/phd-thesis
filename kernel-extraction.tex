...
Figure~\ref{fig:flattening-rules} lists the rules that form the basis
...
and the inner expression.  Presently, nested \lstinline{stream_red}s are
...
to exercise the code generator, but sequentialising \lstinline{stream_red} is
the right thing to do for most of the data sets we use in
Section~\ref{chap:empirical-evaluation}.
...
\begin{lstlisting}[aboveskip=-0.8\baselineskip]
