...
can be mapped to GPU code.  This is of of particular interest since
\cref{chap:kernel-extraction} shows how nested parallelism is
transformed into a form very similar to these primitives.
...
\item[Constant memory] is a size-limited read-only on-chip cache.
  ...
  when many threads are reader the same memory in the same clock
  cycle, as accessing the constant cache is significantly faster terms
  of latency) than accessing global memory.
  ...
  runtime.  Thus, the choice of whether to use constant memory is thus
  a compile-time decision, while the validity of the choice can only
