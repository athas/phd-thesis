...
previous section.  However, making a type \textit{more} existential
would still require us to fix up the relevant patterns.
...
\begin{figure}[bt]
  ...
  \fracc{
    ...
    p = (\nseq{d^p}{l},\nseq{y}{m})
    \\
    \textrm{$S_{r}$ is surjective substitution from $\nseq{d^{p}}{l}$ to $\nseq{d}{k}$} \quad S_r(\rho) = (\nseq{\texttt{i32}}{l}, \nseq{\tau'}{m})
    \\
    \Gamma,p \vd e_2 : \exists\nseq{d^r}{k}.\rho'
  }{
    \Gamma \vd \Let{p}{e_1}{e_2} : \textrm{ext}(\nseq{x}{n}, \exists\nseq{d^r}{k}.\rho')
  }
...
\fracc{
  ...
  \\
  (\exists\seq{z}.\rho') = \textrm{Unification of $\exists\seq{x}.\rho_1$ and $\exists\seq{y}.\rho_2$}
}{
  \Gamma \vd \If{s}{e_1}{e_2} : \exists\seq{z}.\rho'
}
...
program, where all types have size information.
