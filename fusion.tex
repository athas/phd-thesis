...
(and therefore \kw{zip}/\kw{unzip}) do not occur.
...
The types of the new SOACs are shown in \cref{fig:newSoacType}.  An
...
\item[\kw{redomap}] is a composition of \kw{map} and \kw{reduce} that
  ...
  this enables horizontal fusion.  A source-language \kw{reduce} is merely a special case of \kw{redomap}:\\
...
in (ys, zs, sum)
\end{lstlisting}
  This can be fused into a single \kw{redomap}, as follows:
\begin{lstlisting}
...
\Cref{fig:t1t2-1} shows a small flow graph and highlights instances
...
\kw{zip}/\kw{unzip} and using our tupleless SOACs representation:
...
However, this is the worst-case situation: at most we add one \kw{copy}
...
single \kw{map} (see \cref{sec:horizontal-fusion} where we use just
this example).
...
\subsection{List of Fusion Rules}
